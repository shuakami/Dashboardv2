...
export const generateAndSubmitReport = async () => {
  ...
  try {
    ...
    // 构建提示词
    const promptSystem = `生成周报，包含四部分数据分析：今日、昨日对比、本月累计、总计。每部分应包含UV、新访客、PV、跳出率和平均时长。附加线性分析、对比分析和改进方案。格式如下：### 网站数据分析周报 #### 1. 今日数据分析：- UV: [今日UV] - 新访客...以此类推，把1234都说完。然后来到线性分析、对比分析和改进方案部分按照：线性分析：xxx格式`;
